...
/***/ function(module, exports, __webpack_require__) {
	...
	var imgSizer = { Config: { imgCache: [], spacer: "/path/to/your/spacer.gif" }, collate: function collate(aScope) {
					var isOldIE = document.all && !window.opera && !window.XDomainRequest ? 1 : 0;if (isOldIE && document.getElementsByTagName) {
							var c = imgSizer;var imgCache = c.Config.imgCache;var images = aScope && aScope.length ? aScope : document.getElementsByTagName("img");for (var i = 0; i < images.length; i++) {
									images[i].origWidth = images[i].offsetWidth;images[i].origHeight = images[i].offsetHeight;imgCache.push(images[i]);c.ieAlpha(images[i]);images[i].style.width = "100%";
							}
							if (imgCache.length) {
									c.resize(function () {
											for (var i = 0; i < imgCache.length; i++) {
													var ratio = imgCache[i].offsetWidth / imgCache[i].origWidth;imgCache[i].style.height = imgCache[i].origHeight * ratio + "px";
											}
									});
							}
					}
			}, ieAlpha: function ieAlpha(img) {
					var c = imgSizer;if (img.oldSrc) {
							img.src = img.oldSrc;
					}
					var src = img.src;img.style.width = img.offsetWidth + "px";img.style.height = img.offsetHeight + "px";img.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + src + "', sizingMethod='scale')";
					img.oldSrc = src;img.src = c.Config.spacer;
